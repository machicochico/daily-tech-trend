...
## 立場別200文字サマリー仕様（提案）
ユーザが記事を行動に繋げやすくするため、各記事で「技術者・経営者・消費者」の3立場別に、考え方・推奨行動・注意点を含む200文字前後（目安170〜230文字）の要約を生成する。各要約末尾に参考情報（evidence_urls由来のドメイン）を明示し、未取得時はフラグ付けする。既存`perspectives`は互換維持しつつ、`perspective_digest`を追加し段階導入する。
