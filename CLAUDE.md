...
### 出力先 (docs/)
- `docs/index.html`: メインページ（技術トレンド。夜間バッチが `docs/tech/index.html` を root へコピーするため両者は同一）
- `docs/news/index.html`: ニュースページ
- `docs/ops/index.html`: 運用ページ（フィード品質を含む）
- `docs/forecast/`: 未来予測（日付別ページ + 的中一覧）
- `docs/topic/<id>/`: トピック別タイムライン
- `docs/entity/<slug>/`: 企業・製品別ページ
- `docs/diff/`: 昨日との差分
- `docs/exec/`: カテゴリ別エグゼクティブサマリー
- `docs/search.html` + `docs/search-index.json`: 全文検索
- `docs/feed.xml`, `docs/sitemap.xml`, `docs/api/*.json`
- `docs/assets/css/common.css`, `docs/assets/js/common.js`: 共通アセット
- ※意見ページ (`docs/opinion/`) は廃止済み（コミット 7fafd026）

### フィード定義
`src/sources.yaml` にカテゴリとRSSソースを定義。カテゴリIDは英語（`system`, `manufacturing`, `ai`, `security` 等）。

### Ollama環境変数
- `OLLAMA_MODEL`: 使用モデルID（デフォルト: `gpt-oss:20b`）
- `OLLAMA_FALLBACK_MODEL`: 代替モデル
- `OLLAMA_AUTOSTART_CMD`: Ollama自動起動コマンド（デフォルト: `ollama serve`）
- `OLLAMA_AUTOSTART_WAIT_SEC`: 起動待ち秒数（デフォルト `30`）

## CI/CD
- **発行元はローカル（PC2）のみ**: タスクスケジューラ「Daily Tech Trend」→ `C:\work\run_daily.bat`（未git管理）が毎晩パイプラインを実行し、`src/git_auto_push.py` が `docs/` をコミット＆プッシュ
- GitHub Actions (`.github/workflows/ci.yml`) は**検証専用**: push/PR 時に `feed_lint` と `pytest` を実行。リポジトリへの書き込みはしない（旧 daily.yml のパイプライン実行＋コミットは 2026-07 に廃止）
- `src/watchdog.py` がタスクスケジューラから30分間隔で起動し、パイプラインの監視・自動修復を行う

## 注意事項
- `src/` 内のモジュールは `src.` プレフィックスなしで相互importする（`from db import connect`）。実行時のカレントディレクトリは**リポジトリルート**が前提（`db.connect()` が `data/state.sqlite` を相対解決するため。`src/` から実行すると `src/data/` を見てしまう）
- `render_main.py` は約4,400行/175KBの巨大ファイル。HTMLテンプレートがインライン文字列として埋め込まれている（段階的分割中: フィード/検索は `render_feeds.py` へ分離済み）
- `data/` と `logs/` は `.gitignore` 対象。**`state.sqlite` はコミットしない**（2026-07 に追跡解除済み。topic_snapshots は14日retentionで自動パージされる）
- テストは `tests/` に `test_*.py` 形式で配置。外部API依存のテストはモック化されている
- 本リポジトリは夜間パイプライン・watchdog・複数の並行セッションが同一ワーキングツリーを更新する。**着手前に必ず `git status` で未コミット差分を確認する**
- 停止フィードの棚卸しは `python src/feed_recheck.py`（`--apply` で復活フィードの health リセット）
